    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6 py-12 md:py-16">
      ...
      <section class="mb-10">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm space-y-4">
          ...
          <p class="text-xs text-gray-400 pt-1">※ チャンネルの紐付けは会社名に触れると自動提案（手動 <code>/crm-link</code> は上級者向け）。</p>
        </div>
      </section>
...
usePageSeo({
  ...
  description: 'クラゲディールの使い方FAQ・トラブルシューティング。メモが反映されない／担当者で絞りたい／重複登録／削除・修正／DMとチャンネルの違い／コマンド早見表など。',
})
...
const faqs = [
  ...
  { q: 'DMとチャンネル、どちらで使う？', a: '<b>DM</b>（クラゲディールAppとの個別チャット）＝あなただけのプライベート。入力も回答も他の人に見えません。<br><b>共有チャンネル</b>＝メンバー全員に見えます。チームで共有したい案件向け。' },
  { q: 'チームで共有して使うには？', a: 'そのチャンネルで会社名に触れると、クラゲが「🔗 このチャンネルを〇〇に紐付けますか？」と提案します。紐付けると、<b>会話から商談シグナルを自動でメモ</b>します。' },
  { q: '複数人まとめて登録できる？', a: '「ABC商事に田中さん、佐藤さんを登録」のように、読点や「と」で区切れば複数人を一度に登録できます（全員に担当者が付きます）。' },
