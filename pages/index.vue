...
const faqs = [
  ...
  },
  {
    ...
    a: 'まず、クラゲに一度話しかけてみてください（クラゲディール App に DM、または招待済みチャンネルで一言）。初回のやり取りで welcome DM が届き、サイドバーにクラゲディールが常駐します。\nそれでも表示されない場合は、Slack 画面上部の検索窓（または Cmd / Ctrl + K）に「クラゲディール」（英字 kuragedeal でも可）と入力し、表示されたアプリを選んでください。DM が開き、サイドバーに常駐します。反映が遅いときは Slack を再読み込み（Cmd / Ctrl + R）してください。\nなお、ワークスペースへの追加（インストール）は管理者が 1 回行えば完了で、メンバーの個別インストールは不要です。',
  },
  {
